 ...
 * 
 * 有一个新用户，会建一个socket连接， 会启一个线程 socket连接只有在用户退出的时候才会关闭。
 * 
 * @author pjy
 * 
 */
public class SocketServer {
	/**
	 * 每个组只部署一份，Eclipse run as -->java Application
	 * 控制面板\所有控制面板项\程序和功能，勾上telnet客户端 其他同学在dos下执行 telnet 组长ip 15898
	 * 0-1023，端口号最大值65535
	 */
	...
	class Service implements Runnable {
		...
		public Service(Socket socketClient) {
			...
			try {
				...
				msg = "user" + this.socketClient.getInetAddress() + "客户端个数:" + socketList.size() + "线程id:"
		...
		/**
		 * socket对象在内存中。客户端不发请求，服务器端也可以主动向客户端发数据。 http的特点是必须是客户端先发请求，服务器端才能返加数据。
		 */
		public void sendmsg() {
